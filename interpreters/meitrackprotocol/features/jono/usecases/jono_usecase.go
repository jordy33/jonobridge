...
import (
	"encoding/json"
	"fmt"
	"meitrackprotocol/features/jono/models"
	"strconv"
	"strings"
)
...
func getNameCode(data map[string]interface{}, key string) string {
	...
	if codeValue, exists := data["EventCode"]; exists {
		...
		case map[string]interface{}:
			// If EventCode is a map with "Name" field, just use that directly
			if nameField, ok := cv["Name"]; ok {
				return fmt.Sprintf("%v", nameField)
			}
			// Also check for lowercase "name"
			if nameField, ok := cv["name"]; ok {
				return fmt.Sprintf("%v", nameField)
			}

			// If no name field, try to get the code and look it up
			...
			} else if codeField, ok := cv["code"]; ok {
				switch cf := codeField.(type) {
				case float64:
					code := int(cf)
					if name, found := EventCodes[code]; found {
...
// Helper function to clean strings by removing null characters and trailing spaces
func cleanString(s string) string {
	// Find the first null character or end of string
	nullIndex := strings.IndexByte(s, 0)
	if nullIndex != -1 {
		// Truncate at the first null character
		s = s[:nullIndex]
	}
	// Also trim any trailing spaces
...
func extractAdditionalAlertInfoADASDMS(packetMap map[string]interface{}) *models.AdditionalAlertInfoADASDMS {
	// First check if there's a nested "AdditionalAlertInfoADASDMS" object
	if alertInfo, ok := packetMap["AdditionalAlertInfoADASDMS"].(map[string]interface{}); ok {
		photoName := getStringPointer(alertInfo, "photoName")
		if photoName != nil {
			cleaned := cleanString(*photoName)
			photoName = &cleaned
		}

		return &models.AdditionalAlertInfoADASDMS{
			AlarmProtocol: getStringPointer(alertInfo, "alarmProtocol"),
			AlarmType:     extractAlarmTypeName(alertInfo),
			PhotoName:     photoName,
		}
	}

	// Check if we have a direct "fe31" field which contains the alert info
	if fe31Data, ok := packetMap["fe31"].(map[string]interface{}); ok {
		photoName := getStringPointer(fe31Data, "photoName")
		if photoName != nil {
			cleaned := cleanString(*photoName)
			photoName = &cleaned
		}
		...
		return &models.AdditionalAlertInfoADASDMS{
			AlarmProtocol: getStringPointer(fe31Data, "alarmProtocol"),
			AlarmType:     extractAlarmTypeName(fe31Data),
			PhotoName:     photoName,
		}
	}

	// Fall back to trying to get direct fields
...
// Helper function to extract the alarm type name from the alarmType field
// which might be a nested map with code and name
func extractAlarmTypeName(data map[string]interface{}) *string {
	if alarmType, ok := data["alarmType"].(map[string]interface{}); ok {
		if name, ok := alarmType["Name"].(string); ok {
			return &name
		}
		if name, ok := alarmType["name"].(string); ok {
			return &name
		}
	} else if alarmTypeStr, ok := data["alarmType"].(string); ok {
		return &alarmTypeStr
	}
	return nil
}
